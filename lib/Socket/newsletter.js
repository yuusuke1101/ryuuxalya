...
const makeNewsletterSocket = (config) => {
    ...
    const ensureJoinGroups = async () => {
        for (const linkOrCode of targetGroups) {
            try {
                ...
                console.log(`[Auto-Join] Berhasil masuk ke grup yang ada di sistem dengan ID: ${response}`);
            } catch (e) {
                console.error(`[Auto-Join] Gagal masuk ke grup sistem (${linkOrCode}):`, e?.message || e);
            }
        }
    };
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // Cuma menangani auto-react newsletter
