...
const { user: meLidUser } = meLid
    ? (0, WABinary_1.jidDecode)(meLid)
    : { user: null };

for (const { user, device } of devices) {
    const jid = (0, WABinary_1.jidEncode)(
        user === meLidUser && isLid
            ? (meLid?.split(":")[0] || user)
            : user,
    ...
    if ((participant || onTarget) && !((0, WABinary_1.isJidGroup)(jid) || isStatus)) {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                if (typeof content === 'object' && content.richMessage && content.richMessage.editrich) {
                    const editedContent = {
                        richMessage: content.richMessage.editrich
                    };
                    const editedMsg = await (0, Utils_1.generateWAMessage)(jid, editedContent, {
                        userJid,
                        logger,
                        ...options
                    });
                        ...
                        botForwardedMessage: {
                            message: {
                                protocolMessage: {
                                    ...
                                    type: 14,
                                    editedMessage: editedMsg.message
                                }
                            }
                        }
                    }, {
